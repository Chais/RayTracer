...
std::shared_ptr<color> textured_material::shade(const color &lcol,
												const direction &l,
												const normal &n,
												const direction &v,
												const vec2 &pos,
		...
		if (!internal) {
			...
			if (phi > 0) {
				...
				return std::make_shared<color>(
					scale(this->texture->at(static_cast<size_t>(std::round(V*(this->texture->size() - 1)))).at(
						static_cast<size_t>(std::round(U*(this->texture->at(0).size() - 1)))), lcol*(this->diffuse*phi))
						+ lcol*(std::pow(std::max<float>(0.0, dot(v, r)), this->exponent)*this->specular));
			}
		}
	} else
		// Ambient light
		return std::make_shared<color>(scale(this->texture->at(static_cast<size_t>(std::round(
												 V*(this->texture->size() - 1)))).at(static_cast<size_t>(std::round(
