...
hemisphere_light::hemisphere_light(const std::shared_ptr<color> emit_col, const direction &emit_dir,
...
intersection hemisphere_light::intersect_full(const ray &r) const {
	return intersection();
...
bool hemisphere_light::intersect_shadow(const position &o, const direction &d) const {
	return false;
}

const std::shared_ptr<std::vector<ray>> hemisphere_light::shed(unsigned long samples) const {
	random_sampler s;
